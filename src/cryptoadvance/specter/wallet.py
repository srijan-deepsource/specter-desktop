...
class Wallet:
    ...
    def balance_on_label(self, label):
        balancelist = [
            ...
            if self.getlabel(utxo["address"]) == label
        ]
        return sum(balancelist)
    ...
    def labels(self):
        return list(
            dict.fromkeys([self.getlabel(addr) for addr in self.active_addresses])
        )
    ...
    def createpsbt(
        ...
        if fee_rate > 0.0:
            ...
            options["feeRate"] = "%.8f" % round(
                (adjusted_fee_rate * 1000 + 0.5) / 1e8, 8
            )
            r = self.rpc.walletcreatefundedpsbt(
                extra_inputs,  # inputs
                [{addresses[i]: amounts[i]} for i in range(len(addresses))],  # output
